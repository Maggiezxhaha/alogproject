...
- **Detection**: PGD steps backprop through the detector loss (`forward_loss` if available); clean/adv metrics are reported as simple mAP-style overlap using IoU ≥ 0.5. When detectors do not support loss in eval mode, the provided heuristic loss on detection scores is used. Safety evaluation for detection requires an explicit mAP computation callback to be provided when calling `evaluate_safety`.
- **Segmentation**: PGD uses pixel-wise cross-entropy on masks; clean vs. adversarial mIoU are compared. When no callback is provided, a default pixel-accuracy-style score is used to keep evaluation runnable, but providing an explicit mIoU metric is recommended.
